// k6 yük testi — Stock Portfolio Tracker
// Çalıştırma: k6 run perf/load-test.js
// Veya:       BASE_URL=http://localhost:8000 k6 run perf/load-test.js
...
export function setup() {
  const r = http.post(
    `${BASE}/portfolios`,
    JSON.stringify({ name: `perf-${Date.now()}`, currency: "TRY" }),
...
export function handleSummary(data) {
  return {
    "perf/perf-report.json": JSON.stringify(data, null, 2),
